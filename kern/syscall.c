...
sys_env_set_pgfault_upcall(envid_t envid, void *func)
{
	...
	user_mem_assert(env, func, sizeof(void *), 0);
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	page = page_lookup(srcenv->env_pgdir, srcva, &pte_entry);
	if (page == NULL)
		return -E_INVAL;

	if (!(*pte_entry & PTE_W) && (perm & PTE_W))
		return -E_INVAL;
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((uint32_t) srcva < UTOP) {
		...
		pte_entry = pgdir_walk(curenv->env_pgdir, srcva, 0);
		if (pte_entry == NULL)
			return -E_INVAL;
		...
		if ((uint32_t) env->env_ipc_dstva < UTOP) {
			pp = pa2page(PTE_ADDR(*pte_entry));
			r = page_insert(env->env_pgdir, pp, env->env_ipc_dstva,
					perm);
			if (r < 0)
				return r;
...
static int
sys_send(void *data, size_t count)
{
	user_mem_assert(curenv, data, count, 0);

	return e100_send_data(data, count);
}

static int
sys_recv(void *data, size_t count)
{
	user_mem_assert(curenv, data, count, PTE_W);

	return e100_recv_data(data, count);
}
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
		...
		case SYS_page_map:
			return sys_page_map(a1, (void *) a2, a3, (void *) a4,
					a5);
		case SYS_page_unmap:
